...
const sample = (): MotionTelemetry => ({ token: 'run1', contactFailure: null, continuityFailure: null, maxJointVelocity: 0, currentJointSpeed: 0, maxJointAcceleration: 0, sweptFrames: 1, progress: 0, timestamp: 100, tcpError: 0.01, directionError: 0.1, plannedTcpError: 0.01, plannedDirectionError: 0.1, joints: UR20_JOINT_NAMES.map((name) => UR20_READY_JOINTS[name]) })
describe('actual chain cycle acceptance', () => {
  it('rejects missing, stale, mismatched, invalid and failed measurements', () => {
    ...
    for (const value of [null, { ...sample(), contactFailure: 'door hit' }, { ...sample(), sweptFrames: 0 }, { ...sample(), continuityFailure: 'joint jump' }, { ...sample(), timestamp: NaN }, { ...sample(), timestamp: -200 }, { ...sample(), token: 'old' }, { ...sample(), progress: 0.5 }, { ...sample(), tcpError: 0.019 }, { ...sample(), directionError: 0.3 }, { ...sample(), plannedTcpError: Infinity }, { ...sample(), plannedDirectionError: NaN }, { ...sample(), joints: [0] }, { ...sample(), joints: Array(6).fill(20) }]) {
